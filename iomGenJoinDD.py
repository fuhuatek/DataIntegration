...
def expandSource(source,parameter,signal,expandedSources):
    ...
    if source.Granularity == "Container":
        ...
        groupId = parameter
        if source["Selection Criteria"] == "OBJECT_VALID":
            source["Selection Criteria"] = "SOURCE_HEALTH_SCORE"
            if source["Comments"] != None and source["Comments"] != "":
                source["Comments"] += "\n"
            else:
                source["Comments"] = ""
            source["Comments"] += "Changed Selection Criteria to Source Health Score"
...
def adjustValidityCriteria(currParam):
    if currParam.PublisherFunctionalMinRange not in ["TBD","N/A","",None] and \
        ...
        elif currParam.ParameterType == "COD" or currParam.ParameterType == "INT" or currParam.ParameterType == "SINT":
            if currParam.PublisherFunctionalMinRange >= 0:
                currParam.ValidityCriteria += ",RANGE_UINT"
            else:
                currParam.ValidityCriteria += ",RANGE_INT"
        elif "BNR"  in currParam.ParameterType.upper() or "UBNR" in currParam.ParameterType.upper():
...
# ---------------------------------------------------------------------------------------------------------
def parseIdentSheet(filename):
    from openpyxl.reader.excel import load_workbook
    workbook = load_workbook(filename, read_only=True, data_only=True)

    sheet = workbook.get_sheet_by_name("Identification")
...
def convertAlertParams(filename, alertParams, currLRU):
    lineno = 2
    idrecord = parseIdentSheet(filename)

    for alertParam in alertParams:
        if alertParam["Destination LRUs"] is None or alertParam["Destination LRUs"] == "":
            logerr("File: %s Line: %d: 'Destination LRUs' it empty" % (filename, lineno))
        elif not currLRU in alertParam["Destination LRUs"]:
            del alertParam
        else:
            try:
                alertParam.Parameter      = idrecord.system + "_" + alertParam["Logic Statements Parameter Alias"].strip()
                alertParam.SysParameter   = alertParam.Parameter
                alertParam.DataType       = alertParam["Data Type"]
                alertParam.DataSize       = 32  #Data Size is always 32 for FDAS Alert Engine Parameters
                ...
                logerr("File: %s Line: %d: Can't extract parameters: %s" % (filename, lineno, str(emsg)))
		lineno += 1
...
def main(arglist):
    ...
    for f in mapfiles:
        ...
        if sheets is not None:
            ...
            if inMap[0].has_key("Logic Statements Parameter Alias"):
                ...
                convertAlertParams(f, inMap, currLRU)
    ...
    for sig in inputSignals:
        ...
        try:
            sigdict[sig.RpName].ParameterSize=int(sig.ParameterSize)
        except:
            sigdict[sig.RpName].ParameterSize = ""
    ...
    for sig in outputSignals:
        ...
        try:
            sigdict[sig.UniqueName].ParameterSize=int(sig.ParameterSize)
        except:
            sigdict[sig.UniqueName].ParameterSize = ""
#        sigdict[sig.UniqueName].ParameterSize=int(sig.ParameterSize)
