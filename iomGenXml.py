...
from iomGenConst import APPLICATION_RATE, INSTNAME_TO_SDI
...
#set $schedOffset = map.schedOffset
#set $schedRate   = map.schedRate
TEMPLATE_AFDXIN = '''
 #for $key in $model.afdx.messageKeys
  ...
  #set $schedRate   = 1
    ...
    <AfdxMessage id="$map.msgid" messageName="$map.fullname" portType="$map.portType" queueLength="$map.queueLength"
        ...
        schedOffset="$schedOffset" schedRate="$schedRate"/>
  #end if
 #end for

 #for port in $model.a429.ports.values()
    #set namelist=$port.portName.split(".")
    <A429Port id="$port.portId" portName="$namelist[-1]" portType="$port.portType" queueLength="$port.queueLength" length="$port.msgSize" physPort="$port.physPort"/>
 #end for
...
TEMPLATE_AFDXOUT= '''    
 ...
 #for $key in $model.afdx.messageKeys
  ...
  #if $map.msgUsed
        <AfdxMessage id="$map.msgid" messageName="$map.fullname" portType="$map.portType" queueLength="$map.queueLength" 
                     portId="$map.portId" portName="$map.portName" a653PortRefreshPeriod="$map.msgTxRefreshPeriod" length="$map.msgLength"
                     crcOffset="$map.crcOffset" crcFsbOffset="$map.crcFsbOffset" fcOffset="$map.fcOffset" fcFsbOffset="$map.fcFsbOffset"
                     schedOffset="$schedOffset" schedRate="$schedRate"/>
  #end if
 #end for
 ...
 #for key in $model.afdx.datasets.keys()
    ...
    #end if
    #set dsName = ".".join($key)
        <DataSet name="$dsName" dsOffset="$ds[0].dsDsOffset" fsbOffset="$ds[0].dsFsbOffset" messageId="$ds[0].msgRef.msgid" dsType="$dsType" dsA429Label="$ds[0].dsA429Label" dsA429SDI="$dsSDI" dsA429SSMType="$ds[0].dsA429SSMType">
        #for $p in $ds:
            <Parameter paramNameValue="$p.paramName" paramSize="$p.paramDatasize" paramType="$p.paramDatatype" paramNameStatus="$(p.paramName).Status" paramOffsetValue="$p.paramOffset" paramOffsetStatus="$p.paramOffsetStatus">
...
TEMPLATE_CANOUT = '''
 ...
 #for $key in $model.can.messageKeys
  #set $msg=$model.can.messages[$key]
  #if $msg.msgUsed
        <CanMessage id="$msg.msgid" messageName="$msg.fullname" canId="$msg.msgCanID" canbusName="$msg.msgPhysPort" rate="$msg.msgTxRefreshPeriod" length="$msg.msgLength">
...
def genXml(inputModel, outputModel, outfn,appSDI):
    ...
    if outputModel:
        ...
        for tmplstr in (TEMPLATE_AFDXOUT, TEMPLATE_CANOUT):
            tmpl = Template(tmplstr, searchList=[{ "model" : outputModel,"appSDI": appSDI }])
            xmlfile.write(tmpl.respond())
...
def usage():
    sys.stderr.write("Usage: iomGenXml excelicd [--indd inputdd] [--outdd outputdd] [-o outputfile] [-a appname] [-i instancename]")
...
def main(arglist):
    ...
    opts, args = getopt.getopt(arglist, 'ha:i:o:', ['appname=', 'indd=', 'outdd=', 'instancename=','output=', 'help'])
    ...
    if instancename:
        try:
            appSDI  = INSTNAME_TO_SDI[instancename]
        except:
            pass
    ...
    genXml(inputModel, outputModel, xmlfile,appSDI)
    return 0
