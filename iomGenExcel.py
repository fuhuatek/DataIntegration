...
class IcdProcessor():
    ...
    def processOneRP(self, xha, msgtype, xport, dataflowreflist, xrp, hfnamelist):
        ...
        def addA429ContainerRP(rprec, xport, isgw):
            ...
            newPubrefName   = ".".join(rprec.PubrefName.split(".")[:-1])
            txlru           = _getTopNode(xsig)
            ...
            newrp = Bunch(
                status      = 'Initial',
                # we don't have an xrp. Therefore we must make sure that 
                # gatherRxSignalDetails does not need anything from it
                xrp         = None,         
                icdFix      = 'V',
                rpName      = txlru.a.Name + "." + newPubrefName,
                PubrefName  = newPubrefName,
                msg         = rprec.msg,
                dfref       = None,
                dp          = xsig,
                dpclass     = xsigclass,
            ...
            if not isDuplicateRp(rpkey,False):
