...
class AfdxMessage(object):
    ...
    def __init__(self, msgid, afdxMsg, direction, apprate=0):
        ...
        #try:        
            #self.msgTxInterval = fval(afdxMsg.TxInterval)
        #except:
        ...
        try:
            self.msgTxRefreshPeriod = fval(afdxMsg.TxRefreshPeriod)
        except:
            raise Exception, "Bad TxRefreshPeriod"
            
        try:
            self.msgA653RefreshPeriod = fval(afdxMsg.A653PortRefreshPeriod)
        except:
        ...
        if self.direction == "input":
            try:
                self.msgRxSamplePeriod = fval(afdxMsg.RxSamplePeriod)
            except:
                raise Exception, "Bad RxSamplePeriod"
            
            try:
                self.msgInvalidConfirmationTime = 4*fval(afdxMsg.InvalidConfirmationTime)/3                
            except:
                raise Exception, "Bad InvalidConfirmationTime"
                
            try:
                self.msgValidConfirmationTime = fval(afdxMsg.ValidConfirmationTime)/3
            except:
        ...
        if direction == "input":
            ...
            if apprate != 0 and int(self.msgTxRefreshPeriod) >= int(apprate):
                # compute receive schedule rate 
                self.schedRate = int(self.msgTxRefreshPeriod) / int(apprate) 
            else:
                self.schedRate = 1
...
class CanMessage(object):
    ...
    def __init__(self, msgid, canMsg, direction):
        ...
        #try:
            #self.msgRate    = fval(canMsg.TxInterval)
        #except:
            #raise Exception, "Bad value for CAN Message TxInterval"
        
        try:
            self.msgTxRefreshPeriod = fval(canMsg.TxRefreshPeriod)
        except:
            raise Exception, "Bad TxRefreshPeriod"
            
        if self.direction == "input":
            try:
                self.msgRxSamplePeriod = fval(canMsg.RxSamplePeriod)
            except:
                raise Exception, "Bad RxSamplePeriod"
                
            try:
                self.msgInvalidConfirmationTime = 4*fval(canMsg.InvalidConfirmationTime)/3
            except:
                raise Exception, "Bad InvalidConfirmationTime"
                
            try:
                self.msgValidConfirmationTime = fval(canMsg.ValidConfirmationTime)/3
            except:
                raise Exception, "Bad ValidConfirmationTime"
...
class A429Port(object):
    def __init__(self, portName, portId, physPort, direction, portType="QUEUING", queueLength=1):
...
class A429Label(object):
    ...
    def __init__(self, msgid, a429Label, direction, portDict):
       ...
       # try:
            #self.msgRate        = fval(a429Label.TxInterval)
        #except:
            #raise Exception, "Bad value for A429 Label transmission TxInterval"
        try:
            self.msgLabel       = int(a429Label.LabelID) # Octal
        except:            
        ...
        try:
            self.msgTxRefreshPeriod = fval(a429Label.TxRefreshPeriod)
        except:
            raise Exception, "Bad TxRefreshPeriod"

        if self.direction == "input":
            try:
                self.msgRxSamplePeriod = fval(a429Label.RxSamplePeriod)
            except:
                raise Exception, "Bad RxSamplePeriod"
                    
            try:
                self.msgInvalidConfirmationTime = 4*fval(a429Label.InvalidConfirmationTime)/3
            except:
                raise Exception, "Bad InvalidConfirmationTime"
                
            try:
                self.msgValidConfirmationTime = fval(a429Label.ValidConfirmationTime)/3
            except:
                raise Exception, "Bad ValidConfirmationTime"            
...
class OutputSignal(object):
	...
	self.dsA429Label      = ""
        ...
        try:
            self.validityCriteria            =sval(outputSignal.ValidityCriteria)
        except:
            self.validityCriteria = "" 
        
	if self.validityCriteria.find("SSM") != -1:
            try:
                self.dsA429Label = int(self.DpName.split(".")[-2][1:4],8)
            except:
                self.dsA429Label =""
            
            ssmposstart=self.validityCriteria.find("SSM")
            ssmposend=ssmposstart+7
            a429ssmvalidity=self.validityCriteria[ssmposstart:ssmposend]
            self.dsA429SSMType = {"SSM_BNR":"BNR","SSM_DIS":"DIS","SSM_BCD":"BCD","SSM_CUSTOM":"NONE"}[a429ssmvalidity]
        if self.sigType == "BCD":
            try:
...
class MapReader(object):
    ...
    def readSources(self, sourceTable):
        ...
        def mkCond(seltype, sigmap):
            if seltype == "A429SSM":
                ...
                if valflg & VALIDITY_A429SSM_BCD:
                    ...
                    logerror("Sources", sigmap.paramName, "No SSM for parameter")
                    return None
        ...
        for source in sourceTable:
            ...
            busType = self.sourceXref[src.sourceName][0].busType
            if busType != "A825":
            ...
            if src.selectionCriteria == "OBJECT_VALID":
                ...
                if src.granularity == "Message":
                    ...
                    if map0.busType == "A664":
                       src.conditions = [mkCond("FRESH", map0), 
                                         mkCond("A664FS", map0)]
                       A429Cond = mkCond("A429SSM", map0) 
                       if A429Cond != None:
                           src.conditions.append(A429Cond)
                           
                    elif map0.busType == "A429":
                       src.conditions = [mkCond("FRESH", map0),
                                         mkCond("A429SSM", map0)]
                    elif map0.busType == "A825":
                        # Do nothing as designed
                        pass
                    else:
                        logerror("Sources", src.sourceName, "Unknown busType of related parameters")
