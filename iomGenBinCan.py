...
def buildConditions(xmllogic, byteoffsetSig, bitlenSig, lsbSig, accessSig, scaleSig):
    ...
    for x in xmllogic.iterfind("condition"):
        ...
        if useFloat == 1:
            cond = struct.pack(ENDIAN + "hhIIIIIIfff", msgIdx, transportId, offset, access, 0, 0, sizeBits, offBits, scale, minf, maxf)
        else:
            cond = struct.pack(ENDIAN + "hhIIIIIIfii", msgIdx, transportId, offset, access, 0, 0, sizeBits, offBits, scale, min, max)
...
def buildCanInputParam(xpar, msgIdx):
    ...
    param = struct.pack (ENDIAN + "iiihhii",
                                valueOffset,
                                statusOffset,
                                paramSize,
                                nofInputs,
...
def buildCanInputMessage(xmsg, msgIdx):
    ...
    validConfirmationTime   = float(xmsg.attrib["validConfirmationTime"])     # freshness in millisecs eg. 50 = 50millisecs
    invalidConfirmationTime = float(xmsg.attrib["invalidConfirmationTime"])   # unfreshness in millisecs eg. 50 = 50millisecs
    ...
    header = struct.pack(ENDIAN + "iIffhbb", 
        int(xmsg.attrib["id"]),
...
def buildCanOutputMessage(xmsg):
    ...
    header = struct.pack(ENDIAN + "iIffhbb", 
        int(xmsg.attrib["id"]),
        int(xmsg.attrib["canId"]),
        float(xmsg.attrib["rate"]),
		float(xmsg.attrib["rate"]),#same sturct with input
        20 + len(mappings),         # size of structure plus mappings 
        int(xmsg.attrib["length"]),
        count,
