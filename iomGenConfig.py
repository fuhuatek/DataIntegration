...
def usage():
    print 'Usage: iomGenConfig [--littleendian|--bigendian] [--steps XBV] [--sol pseudoPort|hddPort|imaCVT] [--indd modelinputdd] [--outdd modeloutputdd] -o outputdir --app appname --inst instancename excelicd'
    sys.exit(1)
...
def main():
    ...
    opts, args = getopt.getopt(sys.argv[1:], 'ho:a:i:', \
                            ['littleendian', 'bigendian', 'xxendian', 'steps=', 'indd=', 'outdd=', 'outdir=', 'app=','inst=', 'help'])
    ...
    if 'X' in steps:
        ...
        for x in outputddfiles:
            arglist += ['--outdd', x] 
        if iomGenXml.main(arglist + ['-o', xmlfile, '-a', appname,'-i', instname,  excelfile]) != 0:
